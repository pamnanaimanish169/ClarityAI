...
function isDuplicateContactError(error: unknown): boolean {
  ...
  const values = Object.values(error).map((v) => String(v).toLowerCase());
  const combined = values.join(" ");
  return combined.includes("already") && (combined.includes("exists") || combined.includes("subscribed"));
}
...
export async function POST(request: Request) {
  ...
  if (!audienceId) {
    return Response.json(
      { error: "Missing RESEND_PRO_AUDIENCE_ID configuration." },
  ...
  try {
    ...
    if (error && !isDuplicateContactError(error)) {
      return Response.json({ error: "Failed to join waitlist." }, { status: 502, headers: cors });
    }
